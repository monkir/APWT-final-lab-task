...
export default function editcustomer({data1}) {
  ...
  return (
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <input type="text" name="email"  placeholder="email" onChange={handleInput} {...register("email", {required: {value: true, message: "Email is required"}, pattern:{value: /^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g, message:"Input must be an email"}})} />
      <span>{errors.email?.message}</span><br/>
      <input type="tel"  placeholder="phone" name="phone" onChange={handleInput} {...register("phone", {required: {value: true, message: "Phone is required"}, pattern:{value: /^\d+$/, message: "phone must me numerical"}})} />
      <span>{errors.phone?.message}</span><br/>
      <textarea name="address" onChange={handleInput} {...register("address", {required: {value: true, message: "address is required"}})} />
